...
def _per_class_accuracy(logits, labels, num_classes):
    ...
    for c in range(num_classes):
        ...
        n = int(mask.sum().item())
        if n > 0:
            acc[c] = (preds[mask] == c).float().mean().item()
    return acc
...
def test(model, testloader, epoch, transform, args, session):
    ...
    if session > 0:
        ...
        per_class_acc = _per_class_accuracy(all_logits, all_labels, test_class)
        seen_acc = float(per_class_acc[:args.base_class].mean())
        unseen_acc = float(per_class_acc[args.base_class:].mean())
...
def adaptive_logit_adjust(model, testloader, epoch, transform, args, session, result_list=None):
    ...
    tau      = getattr(args, 'adj_tau', 0.04)
    lam_base = getattr(args, 'adj_lambda_base', 0.4)
    t_th     = getattr(args, 'adj_entropy_thresh', 0.06)
    T_B      = getattr(args, 'adj_T_base', 0.25)
    a        = getattr(args, 'adj_entropy_alpha', 8.0)
    ...
    if session > 0:
        ...
        seen_acc = float(per_class_acc[:args.base_class].mean())
        unseen_acc = float(per_class_acc[args.base_class:].mean())
        harm = (2 * seen_acc * unseen_acc / (seen_acc + unseen_acc)) if (seen_acc + unseen_acc) > 0 else 0.0
